...
const decorateCommit = (commit, { host, owner, repository }) => {
  const { hash, references } = commit
  if (!commit.message) console.error(`Commit is missing a message!`, commit)
...
const changelogWriterConfigs = {
  ...
  finalizeContext: (context /* , options, commits, keyCommit */) => {
    ...
    return {
      ...
      commitGroups: context.commitGroups.map(commitGroup => ({
        ...commitGroup,
...
module.exports = { changelogWriterConfigs }
