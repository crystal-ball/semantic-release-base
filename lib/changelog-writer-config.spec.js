...
const { changelogWriterConfigs } = require('./changelog-writer-configs')
...
const generateReleaseNotes = (context, commits) =>
  new Promise(resolve => {
    ...
    commitsStream
      .pipe(conventionalChangelogWriter(context, changelogWriterConfigs))
      .pipe(new MyWritable())
...
describe('Changelog writer configs - Commit groups', () => {
  test.only('The commit groups are rendered correctly', async () => {
    const releaseNotes = await generateReleaseNotes(gnContext(), [
